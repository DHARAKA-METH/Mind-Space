...
export const buildChatMessages = (previousMessages = []) => {
  ...
  previousMessages.forEach((message) => {
    ...
    // DeepSeek does not accept empty assistant/user content
    if (!content) {
      console.log("Skipping invalid chat message:", message);
      return;
    }
...
export const getAiResponseMessage = async (userId, roomId) => {
  try {
    ...
    if (
      ...
      throw new Error(
        "DeepSeek returned an empty AI response"
      );
    }
    ...
    console.log(
      "Error getting DeepSeek AI response:",
      error
    );
